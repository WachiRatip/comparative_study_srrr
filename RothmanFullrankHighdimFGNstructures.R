...
genData <- function(n,p,q,r,bprob.V,bprob.W,rho.x,hurst) {
  # create the sparse coefficient matrix C from U, V and W matrices
  U <- matrix(rnorm(p*q,mean=0,sd=1), p, q)
  V <- matrix(rbinom(p*q, size=1, bprob.V), p, q)
  W <- rbinom(p, size=1, bprob.W)
  C <- U * V * W
  ...
  # Fractional Gaussian Noise function; 
  # e_ij = 0.5*(|k+1|^2H + 2|k|^2H + |k-1|^2H) when k=|i-j| in {0,1,2,3,...,q-1}
  error.Sigma <- 0.5*( (abs(abs(t(replicate(q, seq(q)))-seq(q))+1))^(2*hurst)
                       - 2*abs(t(replicate(q, seq(q)))-seq(q))^(2*hurst)
                       + (abs(abs(t(replicate(q, seq(q)))-seq(q))-1))^(2*hurst) )
...
ols.model <- function(C,X,Y,nresponse) {
  ...
  specificity <- sum((rowSums(C.est==0)==nresponse)&(rowSums(C==0)==nresponse))/sum(rowSums(C==0)==nresponse)
...
for (idx in 1:numSim) {
  # generate data
  data <- genData(n,p,q,r,bprob.V,bprob.W,rho.x,hurst)
  C <- data$C
  X <- data$X
  Y <- data$Y
...
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,hurst,"RothmanFullrankHighdimFGNstructures.csv", sep="-")
...
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,hurst,"RothmanFullrankHighdimFGNstructuresMEAN.csv", sep="-")
write.csv(df.mean, save.path, row.names = FALSE)
colnames(df.sd) <- c("model", "sdMSE", "sdSPEC", "sdSENS")
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,hurst,"RothmanFullrankHighdimFGNstructuresSD.csv", sep="-")
