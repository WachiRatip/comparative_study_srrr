...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangLowrankLowdimFGNstructures.csv", sep="-")
...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangLowrankLowdimFGNstructuresMEAN.csv", sep="-")
write.csv(df.mean, save.path, row.names = FALSE)
colnames(df.sd) <- c("model", "sdMSE", "sdSPEC", "sdSENS")
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangLowrankLowdimFGNstructuresSD.csv", sep="-")
