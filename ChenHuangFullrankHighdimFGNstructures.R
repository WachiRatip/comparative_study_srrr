...
sofar.model <- function(C,X,Y,nresponse,nrank) {
  # build a model
  sofarfit <- sofar(Y, X, nrank = nrank, ic.type="BIC")
  C.est <- coef(sofarfit)
  # model evaluation: MSE
  mse <- mse(C, C.est)
  # model evaluation: specificity; ratio between the number of correct deletion
...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"ChenHuangFullrankHighdimFGNstructures.csv", sep="-")
...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"ChenHuangFullrankHighdimFGNstructuresMEAN.csv", sep="-")
write.csv(df.mean, save.path, row.names = FALSE)
colnames(df.sd) <- c("model", "sdMSE", "sdSPEC", "sdSENS")
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"ChenHuangFullrankHighdimFGNstructuresSD.csv", sep="-")
