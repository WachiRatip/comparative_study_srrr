...
# need to be fixed C is not element-wise sparse.

genData <- function(n,p,q,r,p.0,rho.x,hurst) {
...
for (idx in 1:numSim) {
  # generate data
  data <- genData(n,p,q,r,p.0,rho.x,hurst)
  C <- data$C
  X <- data$X
  Y <- data$Y
...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangFullrankHighdimFGNstructures.csv", sep="-")
...
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangFullrankHighdimFGNstructuresMEAN.csv", sep="-")
write.csv(df.mean, save.path, row.names = FALSE)
colnames(df.sd) <- c("model", "sdMSE", "sdSPEC", "sdSENS")
save.path <- paste(n,p,q,r,p.0,rho.x,hurst,"RothmanChenHuangFullrankHighdimFGNstructuresSD.csv", sep="-")
