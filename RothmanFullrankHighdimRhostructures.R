...
genData <- function(n,p,q,r,bprob.V,bprob.W,rho.x,rho.e) {
...
for (idx in 1:numSim) {
  # generate data
  data <- genData(n,p,q,r,bprob.V,bprob.W,rho.x,rho.e)
  C <- data$C
  X <- data$X
  Y <- data$Y
...
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,rho.e,"RothmanFullrankHighdimRhostructures.csv", sep="-")
...
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,rho.e,"RothmanFullrankHighdimRhostructuresMEAN.csv", sep="-")
write.csv(df.mean, save.path, row.names = FALSE)
colnames(df.sd) <- c("model", "sdMSE", "sdSPEC", "sdSENS")
save.path <- paste(n,p,q,r,bprob.V,bprob.W,rho.x,rho.e,"RothmanFullrankHighdimRhostructuresSD.csv", sep="-")
